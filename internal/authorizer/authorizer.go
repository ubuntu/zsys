...
func (a Authorizer) isAllowed(ctx context.Context, action Action, pid int32, uid uint32) bool {
	if uid == 0 {
		...
		log.Debug(ctx, "Any user always authorized")
		return true
	}

	f, err := os.Open(filepath.Join(a.root, fmt.Sprintf("proc/%d/stat", pid)))
	if err != nil {
		log.Errorf(ctx, "Couldn't open stat file for process: %v", err)
		return false
	}
	defer f.Close()
	...
	if err != nil {
		log.Errorf(ctx, "Couldn't determine start time of client process: %v", err)
		return false
	}
	...
	if err != nil {
		log.Errorf(ctx, "Call to polkit failed: %v", err)
		return false
	}

	log.Debugf(ctx, "Polkit call result, authorized: %t", result.IsAuthorized)
